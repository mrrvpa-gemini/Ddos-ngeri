#!/usr/bin/env python3
# Tools Load Testing & Network Stress Test - FIXED VERSION
# Author: RVPA
# Usage: python3 rvpa_stress_fixed.py
...
class RvpaStressTest:
    ...
    def print_banner(self):
        ...
        print(Colors.RED + "║" + Colors.YELLOW + "              RVPA NETWORK STRESS TEST TOOLS v3.0             " + Colors.RED + "║" + Colors.END)
        print(Colors.RED + "║" + Colors.CYAN + "         [ AUTHOR: RVPA | MODE: DENDAM | FIXED ]            " + Colors.RED + "║" + Colors.END)
        print(Colors.RED + "╚══════════════════════════════════════════════════════════════╝" + Colors.END)
        print()
    ...
    def get_input(self):
        ...
        # FIX: Resolve DNS dulu untuk cek validitas target
        try:
            resolved = socket.gethostbyname(self.target)
            print(Colors.GREEN + f"[+] Target resolved: {resolved}" + Colors.END)
        except socket.gaierror:
            print(Colors.RED + "[!] ERROR: Target tidak valid atau tidak bisa diresolve DNS!" + Colors.END)
            print(Colors.YELLOW + "[*] Coba pakai IP langsung atau cek domain" + Colors.END)
            sys.exit(1)
        ...
        # FIX: Batasi thread maksimal 500 untuk hindari system overload
        print(Colors.YELLOW + "[?] Jumlah Thread (max 500, default 100): " + Colors.END, end="")
        thread_input = input().strip()
        if thread_input:
            self.threads = min(int(thread_input), 500)
            if int(thread_input) > 500:
                print(Colors.RED + "[!] Thread dibatasi ke 500 (system limit)" + Colors.END)
        ...
        print(Colors.YELLOW + "[?] Durasi test detik (0=infinite, default 60): " + Colors.END, end="")
    ...
    def generate_payload(self):
        ...
        paths = ["/", "/index.html", "/api/", "/login", "/admin", "/home", "/search"]
            ...
            try:
                ...
                with self.lock:
                    ...
                    if self.counter % 100 == 0:
                        print(Colors.GREEN + f"[+] Thread-{thread_id} | Requests: {self.counter} | Status: ACTIVE" + Colors.END)
                        
                sock.close()
                time.sleep(self.delay)
                
            except Exception as e:
                # FIX: Hindari spam error, hanya print setiap 50 error
                with self.lock:
                    if thread_id % 50 == 0:
    ...
    def start_test(self):
        ...
        print(Colors.CYAN + f"Duration    : {Colors.WHITE}{self.duration}s {'(Infinite)' if self.duration == 0 else ''}" + Colors.END)
        ...
        try:
            if self.duration > 0:
                ...
                # Infinite mode, tunggu sampai CTRL+C
                while True:
                    time.sleep(1)
